...
(function () {
    ...
    function campaignLevelConfig(chapter, levelInChapter) {
        ...
        var width = size;
        var height = size;
        if (chapter >= 9 && levelInChapter % 5 === 0 && width < 9)
            width += 1;
        if (chapter >= 13 && levelInChapter % 7 === 0 && height < 9)
            height += 1;
        var states;
        if (chapter <= 2)
            states = 2;
            ...
            states = levelInChapter % 3 === 0 ? 4 : 3;
        else if (chapter <= 24)
            states = levelInChapter % 2 === 0 ? 5 : 3;
        else if (chapter <= 26)
            states = levelInChapter % 3 === 0 ? 4 : 5;
        else
            states = levelInChapter % 4 === 0 ? 4 : 5;
        var pattern = "cross";
        if (chapter >= 7 && chapter <= 8)
            pattern = levelInChapter % 2 ? "diagonal" : "cross";
        if (chapter >= 9 && chapter <= 10)
            pattern = levelInChapter % 2 ? "horizontal" : "vertical";
        if (chapter >= 11 && chapter <= 13)
            pattern = levelInChapter % 3 === 0 ? "square" : "cross";
        if (chapter >= 14 && chapter <= 16)
            pattern = levelInChapter % 4 === 0 ? "knight" : "diagonal";
        if (chapter >= 17)
            pattern = levelInChapter % 3 === 0 ? "randomMixed" : ["cross", "diagonal", "square", "horizontal", "vertical", "knight"][levelInChapter % 6];
        if (chapter >= 22)
            pattern = levelInChapter % 2 ? "randomMixed" : pattern;
