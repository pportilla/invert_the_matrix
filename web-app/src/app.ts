...
(function () {
  ...
  var TEXT_TRANSLATIONS = {
    ...
    fr: {
      ...
      "A true inverse matrix can exist only when \\(A\\) is square, meaning it has the same number of rows and columns. This happens on a \\(w\\times h\\) board with no tiles with lock icons and no empty holes, when there is exactly one allowed tap for each tile. In that case \\(A\\) sends vectors in \\(R^{wh}\\) to vectors in \\(R^{wh}\\), and invertibility means every starting board has one unique tap-count solution.": "Une vraie matrice inverse ne peut exister que si \\(A\\) est carrée, c'est-à-dire si elle a le même nombre de lignes et de colonnes. Cela arrive sur un plateau \\(w\\times h\\) sans tuiles avec cadenas et sans trous vides, quand il existe exactement un toucher autorisé pour chaque tuile. Dans ce cas, \\(A\\) transforme des vecteurs de \\(R^{wh}\\) en vecteurs de \\(R^{wh}\\), et son inversibilité signifie que tout plateau de départ a une solution unique comme vecteur de touchers.",
  ...
  function handleAction(action, button) {
    ...
    if (action === "start-playground") startPlayground();
    if (action === "clear-playground") clearPlaygroundBoard();
    if (action === "copy-playground-seed") copyPlaygroundSeed();
    if (action === "load-playground-seed") loadPlaygroundSeedFromInput();
  ...
  function normalizePlaygroundPrefs(progress) {
    ...
    if (!root.playgroundPrefs) root.playgroundPrefs = clone(DEFAULT_PROGRESS.playgroundPrefs);
    ...
    if (PLAYGROUND_PATTERNS.indexOf(prefs.pattern) === -1) prefs.pattern = "cross";
    if (!isValidPlaygroundTool(prefs.tool, prefs.states)) prefs.tool = PLAYGROUND_TOOL_TAP;

    var total = prefs.width * prefs.height;
    var board = Array.isArray(prefs.board) ? prefs.board.slice(0, total) : [];
    while (board.length < total) board.push(0);
  ...
  function cleanPlaygroundIndexes(items, total) {
    ...
    (Array.isArray(items) ? items : []).forEach(function (item) {
      var idx = Number(item);
      if (!Number.isFinite(idx) || idx < 0 || idx >= total || seen.has(idx)) return;
      seen.add(idx);
      cleaned.push(idx);
    });
  ...
  function renderPlaygroundControls() {
    if (!els.playgroundEditorBoard) return;
    var prefs = normalizePlaygroundPrefs();
    els.playgroundWidth.value = prefs.width;
    els.playgroundHeight.value = prefs.height;
    if (els.playgroundStateSummary) els.playgroundStateSummary.textContent = prefs.states + " " + t("states");
    if (els.playgroundPatternSummary) els.playgroundPatternSummary.textContent = patternDisplayName(prefs.pattern);
    if (els.playgroundSizeSummary) els.playgroundSizeSummary.textContent = prefs.width + "x" + prefs.height;
    if (els.playgroundCodeSummary) els.playgroundCodeSummary.textContent = t("Share");
  ...
  function renderPlaygroundEditorBoard(pulsedIndexes) {
    ...
    for (var y = 0; y < prefs.height; y += 1) {
      for (var x = 0; x < prefs.width; x += 1) {
        ...
        if (locked.has(idx)) classes.push("locked");
        ...
        html.push(
          '<button class="' + classes.join(" ") + '" data-playground-index="' + idx + '" data-number="' + STATE_NUMBERS[state] + '" aria-label="' + escapeAttribute(t("Row") + " " + (y + 1) + ", " + t("column") + " " + (x + 1) + ", " + t("state") + " " + state + (locked.has(idx) ? ", " + t("locked") : "")) + '">' +
          lockMark +
          '</button>'
        );
      }
  ...
  function updatePlaygroundSize() {
    ...
    for (var y = 0; y < Math.min(oldHeight, prefs.height); y += 1) {
      for (var x = 0; x < Math.min(oldWidth, prefs.width); x += 1) {
        ...
        if (oldLocked.has(oldIdx)) nextLocked.push(nextIdx);
        if (oldDisabled.has(oldIdx)) nextDisabled.push(nextIdx);
      }
    }
  ...
  function setPlaygroundStatus(message) {
    if (els.playgroundSeedStatus) els.playgroundSeedStatus.textContent = message ? t(message) : "";
  }
  ...
  function normalizedPlaygroundSeedPrefs(prefs) {
    ...
    if (disabled.length >= total) disabled = disabled.slice(0, Math.max(0, total - 1));
  ...
  function decodePlaygroundCompactSeed(seed) {
    ...
    for (var prefixIndex = 0; prefixIndex < prefixes.length; prefixIndex += 1) {
      ...
      for (var start = 0; start <= code.length - prefix.length - 3; start += 1) {
        if (code.slice(start, start + prefix.length) !== prefix) continue;
  ...
  function decodePlaygroundCompactCode(code, prefix) {
    if (!code || code.slice(0, prefix.length) !== prefix || code.length < prefix.length + 3) return null;
    var body = code.slice(prefix.length, -2);
    var checksum = code.slice(-2);
    if (!body || checksum.length !== 2 || compactChecksumText(body) !== checksum) return null;
    var bits = [];
    for (var i = 0; i < body.length; i += 1) {
    ...
    if (compactAlphabetIndex(checksum[0]) < 0 || compactAlphabetIndex(checksum[1]) < 0) return null;
    var position = 0;
    var failed = false;
    ...
    if (failed || version !== PLAYGROUND_COMPACT_VERSION) return null;
    var states = STATE_OPTIONS[stateIndex];
    var pattern = PLAYGROUND_PATTERNS[patternIndex];
    if (!states || !pattern) return null;
    var total = width * height;
    ...
    for (var idx = 0; idx < total; idx += 1) {
      ...
      if (isDisabled) {
        ...
        board.push(state);
        if (isLocked) locked.push(idx);
      }
    }
    if (failed || disabled.length >= total) return null;
    if (Math.ceil(position / 5) !== body.length) return null;
    for (; position < bits.length; position += 1) {
      if (bits[position]) return null;
    }
    return {
      width: width,
      height: height,
  ...
  function decodePlaygroundLegacySeed(seed) {
    ...
    if (parts.length < 7 || parts[0] !== PLAYGROUND_SEED_PREFIX) return null;
    ...
    if (STATE_OPTIONS.indexOf(states) === -1) return null;
    var pattern = parts[3];
    if (PLAYGROUND_PATTERNS.indexOf(pattern) === -1) return null;
    var total = width * height;
    var boardText = parts[4] || "";
    if (boardText.length !== total || /[^0-4]/.test(boardText)) return null;
    ...
    if (!disabled || disabled.length >= total) return null;
    var disabledSet = new Set(disabled);
  ...
  function decodePlaygroundIndexList(text, total) {
    ...
    for (var i = 0; i < parts.length; i += 1) {
      if (!parts[i]) continue;
      if (!/^[0-9a-z]+$/i.test(parts[i])) return null;
  ...
  function copyPlaygroundSeed() {
    ...
    if (els.playgroundSeed) els.playgroundSeed.value = seed;
    var copied = false;
  ...
  function gameDisplayName(game) {
    ...
    if (game.mode === "playground") return t("Playground");
    return game.name;
  }
  ...
  function campaignLevelsFromData(data) {
    ...
    if (levels.length !== CAMPAIGN_LEVEL_COUNT) throw new Error("Campaign data is incomplete.");
    return levels;
  }
  ...
  function renderCampaignGroupTile(groupIndex, selectedGroup) {
    ...
    if (groupIndex === selectedGroup) classes.push("is-selected");
    if (stars >= CAMPAIGN_GROUP_STAR_TARGET) classes.push("is-open");
  ...
  function renderCampaignGroupMiniGrid(groupIndex) {
    return campaignLevelsForGroup(groupIndex).map(function (level) {
      ...
      if (isCampaignHintMarked(level.levelId)) classes.push("is-hinted");
  ...
  function firstUnlockedCampaignGroup() {
    for (var group = 0; group < CAMPAIGN_GROUP_COUNT; group += 1) {
      if (isCampaignGroupUnlocked(group)) return group;
    }
    return 0;
  }
  ...
  function campaignAdjacentGroups(groupIndex) {
    ...
    var groups = [];
    if (x > 0) groups.push(groupIndex - 1);
    if (x < CAMPAIGN_MAP_SIZE - 1) groups.push(groupIndex + 1);
    if (y > 0) groups.push(groupIndex - CAMPAIGN_MAP_SIZE);
    if (y < CAMPAIGN_MAP_SIZE - 1) groups.push(groupIndex + CAMPAIGN_MAP_SIZE);
    return groups;
  }
  ...
  function isCampaignGroupUnlocked(groupIndex) {
    ...
    if (campaignGroupStars(groupIndex) > 0) return true;
  ...
  function campaignGroupAriaLabel(groupIndex, stars, completed, unlocked) {
    ...
    if (!unlocked) label += ", " + t("locked") + ", " + t("Earn 15 stars in a nearby group.");
  ...
  function startCampaignLevel(levelId) {
    ...
    if (!level) return;
    if (!isCampaignLevelUnlocked(level.campaignIndex)) return;
  ...
  function modeLabel(mode) {
    ...
    if (mode === "playground") return t("Playground");
  ...
  function updateCounters() {
    ...
    if (hintButton) hintButton.disabled = app.currentGame.mode === "playground" || hintCompletionPending || app.currentGame.completed || Date.now() < app.hintCooldownUntil;
  }
  ...
  function completeGame() {
    var game = app.currentGame;
    if (!game || game.completed) return;
    if (game.mode === "playground") return;
    game.hintCompletionPending = false;
    game.completed = true;
  ...
  function personalBestText(game) {
    if (!game) return "-";
    if (game.mode === "playground") return t("No goal");
    if (game.mode === "daily") {
